...
bot: build ., env_file .env, command `python -m uvicorn app.main:app --host 0.0.0.0 --port 8000 --workers 1`, depends_on db(healthy)+redis(healthy), dns 8.8.8.8/8.8.4.4, volumes .:/app + reportsdata:/app/reports. ВАЖНО (СЕТЬ): extra_hosts `api.telegram.org:149.154.167.220` ОБЯЗАТЕЛЕН на этом сервере — DNS-резолв api.telegram.org тут нестабилен/отдаёт адреса без маршрута, БЕЗ хардкода ни одно соединение с Telegram не поднимается (Connection timeout на connect, проверено 2026-07-19). НЕ убирать без рабочей замены резолва. Исходящие форсятся в IPv4 (socket.AF_INET). Таймауты getUpdates раздельные: connect/sock_connect=15с, sock_read=40с, total=60с, polling_timeout=30с (см. app/main.py::_make_bot / _run_polling) — это лечит прежние подвисания «раз через раз» (single IP достижим, но общий timeout=60 не отличал долгий long-poll от мёртвого соединения; теперь мёртвое рвётся за 40с и переоткрывается).

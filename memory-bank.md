...
bot: build ., env_file .env, command `python -m uvicorn app.main:app --host 0.0.0.0 --port 8000 --workers 1`, depends_on db(healthy)+redis(healthy), dns 8.8.8.8/8.8.4.4, volumes .:/app + reportsdata:/app/reports. ВАЖНО (СЕТЬ): extra_hosts `api.telegram.org:149.154.167.220` ОБЯЗАТЕЛЕН на этом сервере — DNS-резолв api.telegram.org тут нестабилен/отдаёт адреса без маршрута, БЕЗ хардкода ни одно соединение с Telegram не поднимается (Connection timeout на connect, проверено 2026-07-19). НЕ убирать без рабочей замены резолва. Исходящие форсятся в IPv4 (socket.AF_INET). Таймауты: session.timeout=30с (ЧИСЛО, не ClientTimeout — aiogram 3.7.0 делает int(session.timeout + polling_timeout), объект ClientTimeout там даёт TypeError), polling_timeout=30с → request_timeout getUpdates = 60с (см. app/main.py::_make_bot / _run_polling). Это лечит прежние подвисания «раз через раз»: single IP достижим, но раньше timeout=60 на обычный запрос давал долгое зависание на мёртвом соединении; теперь мёртвый запрос рвётся за 30с и polling переоткрывает его.
